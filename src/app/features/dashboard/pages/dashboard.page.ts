...
export class DashboardPage {
  private readonly dashboardQuery = inject(DashboardQuery);
  ...
  public async onEventClick(event: DashboardEvent): Promise<void> {
    const sessionDetail = await this.dashboardQuery.getSessionDetail(event.id);
  ...
  private async loadDatesWithEvents(): Promise<void> {
    const dates = await this.dashboardQuery.getAllDatesWithEvents();
  ...
  private async loadEventsPage(page: number = 0): Promise<void> {
    ...
    const sessions = await this.dashboardQuery.getWorkoutSessions(
      startDate,
      endDate,
      offset,
    ...
    const newEvents = sessions.map((s) =>
      this.dashboardQuery.workoutSessionToEvent(s),
    );
    ...
    const totalCount = await this.dashboardQuery.getWorkoutSessionsCount(
      startDate,
      endDate,
    );
