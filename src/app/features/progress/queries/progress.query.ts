...
/**
 * Read-model for the progress screen: aggregations over completed
 * sessions. Pure queries — the page owns the snapshot state.
 */
...
export class ProgressQuery {
  ...
  async getProgressSnapshot(): Promise<ProgressSnapshot> {
    const completedSessions = await this.sessionRepository.getCompleted();
  ...
  async getWorkoutStats(): Promise<WorkoutStats> {
    return this.buildWorkoutStats(await this.sessionRepository.getCompleted());
  }

  async getFrequentWorkouts(limit: number = 5): Promise<FrequentWorkout[]> {
    return this.buildFrequentWorkouts(
      await this.sessionRepository.getCompleted(),
      limit,
    );
  }

  async getExercisePRs(): Promise<ExercisePR[]> {
    return this.buildExercisePRs(await this.sessionRepository.getCompleted());
  }

  async getMuscleDistribution(): Promise<MuscleDistribution[]> {
    return this.buildMuscleDistribution(
      await this.sessionRepository.getCompleted(),
    );
  }
  ...
  private async buildWorkoutStats(
    ...
    let totalVolume = 0;
    await this.sessionRepository.forEachLogOfSessions(sessionIds, (log) => {
