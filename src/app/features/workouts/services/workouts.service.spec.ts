...
describe('WorkoutsService', () => {
  ...
  beforeEach(async () => {
    await resetDatabase();
    service = injectService(WorkoutsService);
  });
  ...
  describe('create', () => {
    it('appends the new workout after the current max order', async () => {
      ...
      const id = await service.create('Leg Day');
  ...
  describe('reorderWorkouts', () => {
    it('persists the new order atomically', async () => {
      ...
      await service.reorderWorkouts([w3.id, w1.id, w2.id]);
  ...
  describe('delete', () => {
    it('cascades to exercises, sessions and set logs', async () => {
      ...
      await service.delete(workout.id);
